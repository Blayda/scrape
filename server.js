  ...
  axios.get("https://www.npr.org/").then(response => {
    ...
    $("article").each(function(i, element) {
      ...
      article.summary = $(element)
        .find(".hp-item .story-text .teaser")
        .text()
        .trim();
      article.link = $(this)
        .find("a")
        .attr("href");
      article.imageLink = $(element)
        .find("img")
        .attr("src");
      ...
      db.Article.init()
        .then(() =>
          db.Article.create(filteredResult, (err, dbArticle) =>
            err ? console.log(err) : console.log(dbArticle)
          )
        )
        // If an error occurred, log it
        .catch(error =>
          error ? console.log(error) : res.send("Scrape Complete")
        );
    });
  });
});
...
app.put(
  ...
  (req, res) => {
    console.log(req.params.id);
    db.Note.remove({ _id: req.params.id }, (err, data) =>
